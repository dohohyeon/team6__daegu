# 🚸 Daegu_Child_Safety_Zone_Traffic_Accident_Analysis  
대구시 어린이보호구역 내 교통사고 분석 (3명)

> 대구시 어린이보호구역의 **교통사고 위험도를 정량화**하고  
> **고위험 스쿨존을 발굴해 정책·시설 개선 인사이트**를 도출한 데이터 분석 프로젝트  
> *Traffic Accident Risk Analysis in Child Safety Zones of Daegu*
...
| 구분 | 주요 항목 예시 |
|------|----------------|
| 어린이보호구역 단위 | 사고 건수, 사고 유형, 도로 폭, 제한 속도, 차로 수, 신호등·횡단보도·과속단속카메라·CCTV 설치 여부 등 |
| 시설·환경 정보 | 보행자 교통섬, 안전표지, 보차분리 여부, 인근 버스정류장·교차로 거리 등 |
| 공간·인구 통계 | 행정동별 인구수, 어린이 인구 비율, 학교·유치원 수, 통학로 위치 등 |

> 여러 공공데이터(교통사고, 도로·교통 인프라, 인구통계)를 스쿨존 ID 기준으로 결합하여  
> **“어린이보호구역 단위의 분석용 마스터 테이블”**을 구축했습니다.
